...
try {{
    ...
    $pt.PivotFields('MesReferencia').Orientation   = 2
    [void]$pt.AddDataField($pt.PivotFields('DiasOperadosMes'), 'Soma DiasOperadosMes', -4157)
    foreach ($f in @('Atendimento','TIPO','Contrato','Modalidade')) {{
        $pt.PivotFields($f).Orientation = 3
    }}
    try {{ $pt.PivotFields('MesReferencia').NumberFormat = 'mmm/yyyy' }} catch {{}}
    foreach ($f in @('TIPO','Atendimento','Modalidade','Contrato')) {{
        try {{
            $sc = $wb.SlicerCaches.Add2($pt, $f)
            [void]$sc.Slicers.Add($wsTD)
        }} catch {{}}
    }}
